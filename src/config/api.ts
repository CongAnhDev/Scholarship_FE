...
Module Provider
 */
export const callCreateProvider = (name: string, address: string, description: string, logo: string) => {
    return axios.post<IBackendRes<IProvider>>('/api/v1/providers', { name, address, description, logo })
}

export const callUpdateProvider = (id: string, name: string, address: string, description: string, logo: string) => {
    return axios.patch<IBackendRes<IProvider>>(`/api/v1/providers/${id}`, { name, address, description, logo })
}

export const callDeleteProvider = (id: string) => {
    return axios.delete<IBackendRes<IProvider>>(`/api/v1/providers/${id}`);
}

export const callFetchProvider = (query: string) => {
    return axios.get<IBackendRes<IModelPaginate<IProvider>>>(`/api/v1/providers?${query}`);
}
